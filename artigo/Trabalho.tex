...
A série temporal de Mackey-Glass\cite{mackeyglass} é modelada pela seguinte equação diferencial:
\begin{center}
$\frac{dx(t)}{dt} = \beta \frac{x(t - \tau)}{1 + x^n(t - \tau)} - \gamma x(t)$
\end{center}
Neste trabalho serão utilizados os parâmetros $\beta = 0.2$, $\gamma = 0.1$, $n = 10$ e $\tau = 30$, de forma que nestas condições a série apresenta um comportamento caótico. O estado inicial da série, ou seja, os valores de $x(t)$ para $t \in [-\tau, 0]$ é definido como $x(t) = 1.2$.

\subsection{Aprendizagem de regras fuzzy}

Utilizando o procedimento descrito em \cite{wangmendel} é possível obter uma base de regras \textit{fuzzy} utilizando uma certa quantidade de pontos (antecedentes) para prever os pontos seguintes (consequentes).
...
\bibitem{estevao} Esmi, Estevao; Sussner, Peter ; Bustince SOLA, Humberto ; Fernandez, Javier. {\it $\theta$-Fuzzy Associative Memories ($\theta$-FAMs)}. IEEE Transactions on Fuzzy Systems, v. 23, p. 1-1, (2015). 

\bibitem{mackeyglass} M.C. Mackey, L. Glass, {\it Oscillation and chaos in physiological control systems}. Science, Vol 197: 287-289, 1977
